...
// CompilerType defines which C/C++ compiler driver to use.
...
// CLIConfig holds configuration for C/C++ compiler CLI validation.
type CLIConfig struct {
	Name           string       // "gcc", "g++", "clang", "clang++"
	Type           CompilerType // GCC, GXX, Clang, or ClangCxx
	SupportsStdlib bool         // True for G++/Clang++
}

// ParsedCLIArgs holds parsed command-line information for C/C++ compilers.
type ParsedCLIArgs struct {
	Language string // From -x flag
	Standard string // From -std flag
	StdLib   string // From -stdlib flag (G++/Clang++ only)
}

// ValidateAndParseArgs validates arguments and extracts runtime information
// for C/C++ compiler drivers.
...
// IsValidCompilerFlag checks if a flag is valid for the given C/C++ compiler.
