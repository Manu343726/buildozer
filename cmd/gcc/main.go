...
var log = logger.NewComponentLogger("gcc-driver")
...
func main() {
	...
	if err := registry.Initialize(ctx); err != nil {
		...
		// Continue anyway - we'll create the job assuming GCC is available
	} else {
		// Verify GCC is available on this system
		gcc := registry.GetGCC(ctx)
		if gcc == nil {
			log.ErrorCtx(ctx, "gcc not found on this system")
			os.Exit(1)
		}
		log.InfoCtx(ctx, "using detected GCC toolchain", "version", gcc.CompilerVersion, "path", gcc.CompilerPath)
	}
...
func createJob(parsed *ParsedArgs) (*v1.Job, error) {
	cppToolchain := &v1.CppToolchain{
		Language: v1.CppLanguage_CPP_LANGUAGE_C,
...
func generateJobID() string {
	return fmt.Sprintf("gcc-%d", os.Getpid())
}
