...
func TestParseCommandLineCompileOnly(t *testing.T) {
	args := []string{"test.cpp", "-c", "-o", "test.o"}
	parsed := ParseCommandLine(args)

	if parsed.Mode != ModeCompileOnly {
		t.Errorf("expected ModeCompileOnly, got %v", parsed.Mode)
	}
	if len(parsed.SourceFiles) != 1 || parsed.SourceFiles[0] != "test.cpp" {
		t.Errorf("expected SourceFiles=[test.cpp], got %v", parsed.SourceFiles)
	}
...
func TestParseCommandLineIncludeAndDefine(t *testing.T) {
	args := []string{"test.cpp", "-I/usr/include", "-D", "DEBUG", "-DVERSION=1"}
	parsed := ParseCommandLine(args)
...
func TestParseCommandLineCompilerFlags(t *testing.T) {
	args := []string{"test.cpp", "-O2", "-Wall", "-fPIC", "-std=c++17"}
	parsed := ParseCommandLine(args)
...
func TestParseCommandLineMultipleSources(t *testing.T) {
	args := []string{"file1.cpp", "file2.cpp", "file3.cpp", "-o", "app"}
	parsed := ParseCommandLine(args)
...
func TestDetectLanguageCpp(t *testing.T) {
	sourceFiles := []string{"main.cpp", "util.cpp"}
...
func TestDetectLanguageCppVariants(t *testing.T) {
	tests := [][]string{
		{"main.cc"},
		{"main.cxx"},
		{"main.c++"},
...
func TestGenerateJobID(t *testing.T) {
	...
	if len(id) < 5 {
		t.Errorf("expected job ID to start with g++-or similar, got %s", id)
	}
}
